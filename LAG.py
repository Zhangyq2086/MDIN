...
# LAGConv: Local-context Adaptive Convolution Kernels with Global Harmonic Bias for Pansharpening(2022)
...
class LACNET(nn.Module):
    def __init__(self):
        ...
        self.head_conv = nn.Sequential(
            LAConv2D(32, 32, 3, 1, 1, use_bias=True),  # 第一个通道数修改,通道数相加+1
            nn.ReLU(inplace=True)
        )
        ...
        self.tail_conv = LAConv2D(32, 31, 3, 1, 1, use_bias=True)  # 第二个通道数修改

        self.upsample = nn.Upsample(scale_factor=4, mode="nearest")  # scale_factor = 4为像素倍数，需要修改
    def forward(self, lms, pan):
        lms = self.upsample(lms)
...
if __name__ == '__main__':
    ...
    N = LACNET()
    # Y= summary(N,[(1,64,64),(8,64,64)],device='cpu')
    HS = torch.randn(1, 31, 32, 32)  # 102
    PAN = torch.randn(1, 1, 128, 128)
    Y = N(HS,PAN)
    print(Y.shape)
    # HS = torch.randn(1, 102, 40, 40)  # 102
    # PAN = torch.randn(1, 1, 160, 160)
